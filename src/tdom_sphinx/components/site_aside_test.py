...
def test_site_aside_renders_static_content(page_context):
    ...
    assert headings[0].get_text(strip=True) == "Quick Links"
    assert headings[1].get_text(strip=True) == "Resources"
    ...
    assert len(links) >= 4
    assert links[0].get_text(strip=True) == "Documentation"
    assert links[0].get("href") == "docs"
    assert links[1].get_text(strip=True) == "API Reference"
    ...
    assert resource_links[0].get_text(strip=True) == "Downloads"
